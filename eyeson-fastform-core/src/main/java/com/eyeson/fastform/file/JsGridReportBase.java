...
import com.eyeson.fastform.common.ExcelUtils;
...
public class JsGridReportBase {
    ...
    public static String MODULE_PATH = "../../admin/exceldemo/module.xls"; // 模板路径
    ...
    public HSSFWorkbook writeSheet(HSSFWorkbook wb, String title,
        ...
        return writeSheet(wb, title, styles, creator, tableData, true);
    }
     ...
     * @param creator      创建人
     * @param tableData    表格数据
     * @param haveFileInfo 文件开头是否包含副标题
     * @throws Exception
     */
    public HSSFWorkbook writeSheet(HSSFWorkbook wb, String title,
                                   HashMap<String, HSSFCellStyle> styles, String creator,
                                   TableData tableData, boolean haveFileInfo) throws Exception {
    ...
    public HSSFWorkbook writeSheet(HSSFWorkbook wb, String title,
                                   HashMap<String, HSSFCellStyle> styles, String creator,
                                   TableData tableData, boolean haveFileInfo, String djq)
            throws Exception {
        ...
        HSSFRow row = sheet.createRow(0);// 创建新行
        HSSFCell cell = row.createCell(0);// 创建新列
        HSSFRow row2 = sheet.createRow(2);// 创建新行
        HSSFCell cell2 = row.createCell(2);// 创建新列
        int rownum = 0;
        cell.setCellValue(new HSSFRichTextString(title));
        HSSFCellStyle style = styles.get("TITLE");
        if (style != null)
        ...
        cell = row.createCell(0);
        cell.setCellValue(new HSSFRichTextString("导出人:"));
        style = styles.get("SUB_TITLE");
        if (style != null)
            cell.setCellStyle(style);
        ...
        cell = row.createCell(2);
        cell.setCellValue(new HSSFRichTextString("导出时间:"));
        style = styles.get("SUB_TITLE");
        if (style != null)
            cell.setCellStyle(style);
        ...
        for (TableDataRow dataRow : dataRows) {
            ...
            for (int i = 0; i < size; i++) {
                TableColumn tc = headerMetaData.getColumns().get(i);
                System.out.println(tc.getDisplay());
                if (!tc.isVisible())
                    continue;
                index++;
    ...
    public void exportToExcel(String title, String creator, TableData tableData)
            throws Exception {
        exportToExcel(title, creator, tableData, true);
    }

    public void exportToExcel(String title, String creator,
                              TableData tableData, String djq) throws Exception {
        exportToExcel(title, creator, tableData, true, djq);
    }
    ...
    public void exportToExcel(String title, String creator,
        ...
        wb = writeSheet(wb, title, styles, creator, tableData, haveFileInfo);// 写入工作表

        String sFileName = title + ".xls";
        System.out.println(URLEncoder.encode(sFileName, "UTF-8"));
    ...
    public void exportToExcel(String title, String creator,
        ...
        wb = writeSheet(wb, title.substring(0, 6), styles, creator, tableData,
                haveFileInfo, djq);// 写入工作表
