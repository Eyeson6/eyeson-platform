...
public class BeanUtil {
    ...
    static {
        ...
        String[] datePattern = {"yyyy-MM-dd", "yyyy-MM-dd HH:mm",
                "yyyy-MM-dd HH:mm:ss"};
     ...
     *
     * @param request 请求对象 // * @param receive 重新得到session中保存的查询条件
     * @return PageParam
     */
    public static PageParam wrapPageBean(HttpServletRequest request)
        ...
        try {
            ...
            String refresh = request.getParameter("refresh");// 翻页刷新参数
            // 1刷新所有，2刷新记录数
     ...
     * @return Object
     */
    public static <T> List<T> wrapBeanList(Class<T> cls,
    ...
    public static <T> List<T> wrapBeanList(Class<T> cls, String jsonArray) {
        ...
        try {
            return mapper.readValue(jsonArray,
                    TypeFactory.collectionType(List.class, cls));
        } catch (Exception e) {
            logger.error(getExceptionMessage(e));
            return null;
        }
...
//				SerializationConfig.Feature.WRITE_NULL_MAP_VALUES);
    ...
    public static boolean isBlank(Map<String, Object> params) {
        ...
        for (String key : params.keySet()) {
            if (key.equalsIgnoreCase("sort") || key.equalsIgnoreCase("rows")
                    ...
                    || key.equalsIgnoreCase("refresh")
                    || key.equalsIgnoreCase("recordCount")
                    || key.equalsIgnoreCase("countSql")
                    || key.equalsIgnoreCase("recordSql")
                    || key.equalsIgnoreCase("queryTime")
                    || key.equalsIgnoreCase("begin")
                    || key.equalsIgnoreCase("end")
                    || key.equalsIgnoreCase("t_name")
                    || key.equalsIgnoreCase("totalPage")
                    || key.equalsIgnoreCase("defaultSort")
                    || key.equalsIgnoreCase("head")
                    || key.equalsIgnoreCase("foot")
                    || key.equalsIgnoreCase("sortOrder")
                    || key.equalsIgnoreCase("tableCounts")) {
                continue;
            }
